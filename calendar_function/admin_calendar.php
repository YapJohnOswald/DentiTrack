...
<h2 class="calendar-title">🗂️ Admin Appointment Calendar</h2>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // NEW: List of all valid statuses for the dropdown
  const VALID_STATUSES = ['approved', 'booked', 'completed', 'cancelled', 'declined'];
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    dateClick: function(info) {
      ...
      fetch('../calendar_function/check_rest_day_admin.php?date=' + info.dateStr)
        .then(res => res.json())
        .then(data => {
          if (data.isRestDay) {
  ...
  searchInput.addEventListener('keyup', function() {
    ...
    fetch('../calendar_function/fetch_patients_list_admin.php?query=' + encodeURIComponent(query))
      .then(res => res.json())
      .then(data => {
  ...
  function performSearch(query) {
    if (!query) return;
    // NOTE: fetch_appointments_by_patient_admin.php is being used here
    fetch(`../calendar_function/fetch_appointments_by_patient_admin.php?search=${encodeURIComponent(query)}`)
      .then(res => res.json())
      .then(data => {
  ...
  function fetchAppointments(date) {
    // NOTE: fetch_appointments_by_date_admin.php is being used here
    fetch('../calendar_function/fetch_appointments_by_date_admin.php?date=' + date)
      .then(res => res.json())
      .then(data => {
  ...
  function appendAppointmentCard(a) {
    ...
    switch (a.status.toLowerCase()) {
        case 'pending':
             statusColor = '#FFD966'; // Yellow (Note: This is defined in the PHP, though commented out in CSS/Legend)
             break;
        case 'approved':
  ...
  function showAppointmentDetails(a) {
    ...
    // ACTION BUTTONS: Now includes a dropdown for full status control.
    actionButtons.innerHTML = '';
    const currentStatus = a.status.toLowerCase();
    
    // --- START NEW STATUS EDITING CONTROL ---
    const statusSelect = document.createElement('select');
    statusSelect.id = 'statusChangeSelect';
    statusSelect.style.cssText = 'padding: 8px; border-radius: 6px; border: 1px solid #ccc; font-size: 15px; vertical-align: middle; margin-right: 10px;';
    
    const applyButton = document.createElement('button');
    applyButton.className = 'btn btn-approve'; 
    applyButton.textContent = 'Apply Status Change';
    applyButton.onclick = () => {
        const newStatus = statusSelect.value;
        if (newStatus && newStatus !== currentStatus) {
            updateStatus(newStatus);
        } else {
            alert('Please select a new status to apply.');
        }
    };
    ...
    VALID_STATUSES.forEach(status => {
        ...
        option.value = status;
        option.textContent = status.charAt(0).toUpperCase() + status.slice(1); // Title case
        if (status === currentStatus) {
            option.selected = true;
            // Disable the current status in the dropdown so they must choose a *new* one
            option.disabled = true; 
        }
        statusSelect.appendChild(option);
    });
    
    // Add elements to the action button container
    const statusControlDiv = document.createElement('div');
    statusControlDiv.style.display = 'flex';
    statusControlDiv.style.alignItems = 'center';
    statusControlDiv.appendChild(statusSelect);
    statusControlDiv.appendChild(applyButton);
    
    actionButtons.appendChild(statusControlDiv);
    // --- END NEW STATUS EDITING CONTROL ---
  }
  ...
  function updateStatus(status) {
    ...
    // This calls the updated update_appointment_status_admin.php
